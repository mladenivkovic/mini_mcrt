...
def gas_temperature(u, mu):
    ...
    T = u * (constants.hydro_gamma - 1) * mu * constants.m_u / constants.kB
...
def internal_energy(T, mu):
    ...
    u = constants.kB * T / (constants.hydro_gamma - 1) / (mu * constants.m_u)
    return u
...
def internal_energy_derivative(T, mu):
    ...
    dudT = constants.kB / (constants.hydro_gamma - 1) / (mu * constants.m_u)
    return dudT
